    ...
    <div className="flex-1 overflow-y-auto px-8 py-6">
      ...
      {content.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {content.map((item) => (
            <div key={item._id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-all flex h-40 group cursor-pointer">
              ...
              <div className="p-5 flex-1 flex flex-col">
                ...
                <p className="text-sm font-medium text-emerald-700 mb-auto">{item.episode}</p>
                
                <div className="mt-4">
                  <div className="flex items-center justify-between text-xs mb-2">
                    <span className="font-semibold text-gray-700">{item.progress}% complete</span>
                  </div>
